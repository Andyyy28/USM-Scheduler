...
Candidate legality includes campus, contiguous duration, breaks, room availability, room capabilities, unit authorization, and active locks. Cross-event instructor/section/room conflicts and distinct-day groups remain solver constraints.
...
- **CP-SAT:** one Boolean selection variable for each legal event/candidate pair; exactly one placement per event; at-most-one occupation for each room, instructor, and section time atom; distinct-day constraints; weighted soft penalty minimized. CP-SAT may prove optimality or infeasibility.
...
Every result passes through the same algorithm-independent validator. It verifies completeness, candidate membership, locks, room conflicts, instructor conflicts, section conflicts, and distinct-day constraints. Conflict counting uses a unique resource/event-pair key, so one collision spanning multiple time atoms is counted once.
