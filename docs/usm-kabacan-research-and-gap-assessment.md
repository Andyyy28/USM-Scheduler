...
| Optional approved faculty-workload rules | Consecutive teaching, preparation, consultation, or daily-load policies can improve usability but are not publicly verified as university-wide rules. | Each workload rule remains disabled until its approved source, owner, scope, effective term, threshold, and hard/soft classification are recorded. Validator tests and objective explanations cover each enabled rule. |
...
These priorities do not add room-capacity optimization, walking distance, individual enrollment conflict optimization, examinations, instructor assignment, multi-campus travel, or an AI chatbot. Those remain outside the agreed thesis baseline.
